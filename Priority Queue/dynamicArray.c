...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if (rightchild < last) // if two children
    {
        small = indexSmallest(heap, leftchild, rightchild, compare); // index smallest child
    ...
    else if (leftchild < last) // if one child
        {
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
    // FIXME: implement
    assert(heap!= NULL);
    int tail = dySize(heap);
    
    int i;
    for(i = tail /2 -1; i>=0; i--)
    {
        adjustHeap(heap,tail, i, compare);
    }
}
...
/**
 * Finds index of smallest value in heap.
 * @param v
 * @param i value to be compared.
 * @param j value to be compared
 * @param compare  pointer to compare function.
 */
int indexSmallest (DynamicArray *v, int i, int j, compareFunction compare)
{
